...
class App {
  ...
  getLocalgovernments(name) {
    ...
    const index = this.locations.findIndex((item) => item.state.name === name);

    if (index === -1) return lgas;

    const states = this.locations[index];

    return states.state.locals;
  }
  getAllLocalGovernment() {
    const locals = [];
    this.locations.forEach((state) => locals.push(...state.state.locals));
    return locals;
  }
  ...
  showAlert(
    ...
    return Alert.alert(title, message, [
      {
        text: "Ok",
        onPress: confirmCallBack ? confirmCallBack : () => {},
      },
      {
        text: "Cancel",
